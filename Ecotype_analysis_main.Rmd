...
# Growth analysis
## Age-specific growth
### Setup and cleanup data for mixed models
...
```{r GLMM start, include=FALSE, message=FALSE, warning=FALSE, echo=FALSE, eval=FALSE}
rm(list = ls())
...
### Significance testing (Log-likelihood ratio tests)
...
### Final age-specific growth model & results
...
### Figures for main effects
```{r abiotic effects figures, eval=FALSE}
...
ggsave("Figures/MS/Ecotype-effects-growth.pdf", Ecogrowth, width = 16, height = 8, dpi = 600, bg = "white")
```
...
```{r run revised model, include=FALSE, eval=FALSE}
rm(list = ls())
...
pl_curves = list()
	for(i in 1:4) {
		pl_curves[[i]] = ggplot(data = eco_plot[eco_id == i]) + 
		  ...
		  ylim(0,3.8)+scale_x_continuous(breaks = seq(1,15,2), labels = seq(1,15,2))+
...
ggsave("Figures/MS/Individual_growth_revised.pdf", Ecocurves3, width = 15, height = 15, units = "cm", dpi = 600, bg = "white")
```
