...
---
**This R markdown file contains all relevant analysis steps of the manuscript titled "Variable habitat use supports fine-scale population differentiation of a freshwater piscivore (northern pike, Esox lucius) along salinity gradients in brackish lagoons", including data preparation of elemental transect data, genetics data and growth data (growth data was produced via OtoJ macro of imageJ´s objectJ plugin). Not included are the assignment of element transect data to individual years, raw data preparation of SIMS and LA-ICPMS analysis and plotting of individual-level transects for evaluation of data quality, outliers or vaterite inclusions (indicated by suspicious values of Sr:Ca & Mg:Ca)**

**All chunks except for the packages chunk have been set to eval = FALSE, so each code chunk can be run individually without having to run the whole script. Note that some chunks do require a file that may be the output of a preceeding code chunk, in which case this chunk has tpo be run first**
...
plot(costras)
points(pnts)
plot(raster$X2020.Month.1)

#error comparison between prediction & measured
valid <- lm(validation[[2]]$validation.data ~ validation[[2]]$predicted)
summary(valid)

errorGen(ipdw, measured.spdf, measured.spdf@data$SAL,
  plot = TRUE, title = "IPDW")

#calculate mean, SD, max and range
mean <- calc(raster, fun = mean)
ssd <- calc(raster, fun = sd)
range <- calc(raster, fun = range)
max <- calc(raster, fun = max)

#mean range
range_absolute <- range$layer.2-range$layer.1

#Write spatial rasters for QGIS or similar, change "Salinity_..." for env. variable that was interpolated
writeRaster(raster, "Data/Environment/Salinity_raster_05-22", format="GTiff", overwrite=T)

#load raster and prep for export
Raster1 <- terra::rast("Data/environment/Salinity_mean_revised.tif")

terra::gdal(drivers=T)

writeRaster(Raster1, "Data/environment/Salinity_mean_revised2.tif",
            overwrite=TRUE,filetype="GTiff",
            gdal=c("COMPRESS=NONE", "TFW=YES"), datatype='FLT8S')
```
...
ggplot(oto.int2,aes(x=Sr*1000, y=d18O))+
  geom_point(size=.5)+
...
save(Fgroup,Bgroup,dataB, dataF, Bseries, Fseries, interpol, clust_B, clust_F, Lagoonplot, FWplot,
     file = "Data/Pike/Clustering/Adults_hierarchical_clusters.RData")
```

```{r cluster assignment to individuals, include=FALSE, eval=FALSE}
rm(list=ls())
load("Data/Pike/Clustering/Adults_hierarchical_clusters.RData")
...
#Script to plot fine-grained cluster solution for lagoon sample (supplement)
rm(list=ls())
...
A <- ggplot(Groupall[Groupall$fish_ID=="BH-01906",])+
  ...
  geom_point(aes(dist, Sr*1000), color="black", alpha=1, size=2)+
  theme_classic()+
  ylim(0, 8)+
  scale_y_continuous(breaks = seq(0,8,1), limits = c(0,8))+
  xlim(0,1800)+
...
A <- ggplot(allpike_pheno[allpike_pheno$phenotype=="FW-resident",])+
  ...
  annotation_custom(grob = text_grob(label = "(N = 27)", size = 8, just = "left"),
                    ymin = 7.8, ymax = 7.8, xmin = -50, xmax = -50)+
  geom_segment(x=-450, y=-0.5, xend=-450, yend=8.5, 
               lineend = "butt", linejoin = "mitre",
               size=2, arrow = arrow(length = unit(0.18, "inches")),
               color="darkorange")+
...
B <- ggplot(allpike_pheno[allpike_pheno$phenotype=="BW-resident",])+
  ...
  annotation_custom(grob = text_grob(label = "(N = 44)", size = 8, just = "left"),
                    ymin = 7.8, ymax = 7.8, xmin = 100, xmax = 100)
...
ggsave("Figures/MS/PDF/Fig2.pdf", Transects2, dpi = 600, width = 16, height = 8, units = "cm", bg = "white")
```
...
#Load phenotype data
Otodata <- fread("Data/Pike/Clustering/Phenotypes-element-transects-all.csv")
...
Otoclusters <- Otodata%>%
  transmute(ID=fish_ID,
            ...
            phenotype=case_when(cluster=="FW low saline|cold"~"FW_resident",
                            ...
                            cluster=="BW high saline|cold"~"BW-resident",
                            cluster=="BW high saline|warm"~"BW-resident"),
            waterbody=case_when(capt%in%c("KB", "SB")~"WRB",
                                capt%in%c("WB", "BEG", "KJB", "GJB")~"NRB",
                                capt=="GB"~"GB",
                                TRUE~"Tributary"))%>%
  distinct()
...
barplot(pike.vif[,3], main = "VIF values", horiz = F, col = "darkgreen", ylim = c(0,10), las = 2)
...
```{r test effects, include=FALSE, echo=FALSE, eval=FALSE}
rm(list = ls())
...
```{r age-specific model summary and table, out.width="120%", dpi=300, include=FALSE, echo=FALSE, eval=FALSE, message=FALSE}
rm(list = ls())
...
```{r age-specific model validation, include=FALSE, echo=FALSE, eval=FALSE}
rm(list = ls())
...
hist(pikes$resid, main = "", xlab = "Residuals")
dev.off()
...
par(mfrow=c(2,3))
plot(pikes$resid~as.factor(pikes$phenotype), ylab = "Residuals", xlab = "", las = 2)
plot(pikes$resid~as.factor(pikes$sex), ylab = "Residuals", xlab = "Sex")
plot(pikes$resid~pikes$lifeyear, ylab = "Residuals", xlab = "Age")
plot(pikes$resid~pikes$res_mean, ylab = "Residuals", xlab = "d18O")
plot(pikes$resid~pikes$Sr_mean, ylab = "Residuals", xlab = "Sr concentration")
dev.off()
```

```{r figures for abiotic main effects, include=FALSE, eval=FALSE}
rm(list = ls())
...
ggsave("Figures/MS/PDF/Fig5.pdf", Tempsal, width = 16, height = 9, units = "cm", dpi = 600, bg = "white")
```

```{r figure for phenotye effect, include=FALSE, eval=FALSE}
rm(list = ls())
...
ggsave("Figures/MS/PDF/Fig6.pdf", phenogrowth, width = 16, height = 8, dpi = 600, bg = "white")
```

```{r figure for genotype effect, eval=FALSE}
rm(list = ls())
...
```{r run ecotype model and save output, include=FALSE, eval=FALSE}
rm(list = ls())
...
growthdata2 <- growthdata%>%
  mutate(ecotype_char = case_when(phenotype_char == "FW-resident" ~ "Freshwater",
                                  phenotype_char == "Anadromous" ~ "Freshwater",
                                  phenotype_char == "Trans-habitat" ~ "Cross-habitat",
                                  phenotype_char == "BW-resident" ~ "Brackish"),
         ecotype_num = case_when(phenotype_char == "FW-resident" ~ 1,
                                  phenotype_char == "Anadromous" ~ 1,
                                  phenotype_char == "Trans-habitat" ~ 2,
                                  phenotype_char == "BW-resident" ~ 3))

fwrite(growthdata2, "Data/Pike/Growth/Pikedata_Stan_eco.csv")
...
standata_growth = list(
	Radmm = growthdata2$rad_mm, 
	Agei = growthdata2$lifeyear,
	N = nrow(growthdata2),
	n_fish = as.integer(length(unique(growthdata2$ID2))),
	n_eco = as.integer(max(growthdata2$ecotype_num, na.rm = T)),
	fish_id = as.integer(growthdata2$ID2))
fishtable = unique(growthdata2[, c("ID2", "ecotype_num")])
fishtable = fishtable[order(fishtable$ID2), ]
standata_growth$eco_id_fish = fishtable$ecotype_num

stancode_i = ("Data/Pike/Growth/gm_gamma_inv_ecotype.stan")
mod_i = stan_model(stancode_i, model_name = "Individual pike growth")

fit_gamma_i = sampling(mod_i, iter = 10000, data = standata_growth, init = init_fun,
					 control = list(adapt_delta = 0.99, max_treedepth = 20), chains = 4, thin = 5)

saveRDS(fit_gamma_i, "Data/Pike/Growth/fit_gamma_i_ecotype.rds")

p = c("LInf", "tZero", "k", "LInf_eco", "tZero_eco", "k_eco")
samps_a = as.array(fit_gamma_i, pars = p)

png("Figures/Supplement/trace_eco.png", w = 1500, h = 1500)
	bayesplot::mcmc_trace(samps_a)
dev.off()
png("Figures/Supplement/intervals_eco.png", w = 1500, h = 1500)
	bayesplot::mcmc_intervals(samps_a)
dev.off()
...
# Ecotype codes: 1 = "Brackish resident"; 2 = "Freshwater resident"; 3 = "Anadromous"; 4 = "Cross-habitat"
	...
	for(i in 1:4) {
		pl_curves[[i]] = ggplot(data = pheno_plot[pheno_id == i]) + 
		  ...
		  ylim(0,3.8)+scale_x_continuous(breaks = seq(1,15,1), labels = c("1","","3","","5","","7","","9","","11","","13","","15"))+
			pl_theme + labs(x="Age", y="Otolith radius (mm)")
	}
	pl_curves$nrow = 2
...
phenocurves3 <- annotate_figure(phenocurves2, 
                              ...
                              left = text_grob(expression("Otolith radius"~("mm")), rot = 90, size = 15))

ggsave("Figures/MS/PDF/Fig7.pdf", phenocurves3, width = 15, height = 15, units = "cm", dpi = 600, bg = "white")
```
...
```{r figure VB curves ecotype, include=FALSE, eval=FALSE}
rm(list = ls())

# Ectype codes: 1 = "Brackish resident"; 2 = "Freshwater resident"; 3 = "Anadromous"; 4 = "Cross-habitat"
fit_gamma_i <- readRDS("Data/Pike/Growth/fit_gamma_i_ecotype.rds")
growthdata <- fread("Data/Pike/Growth/Pikedata_Stan_eco.csv")

mypallette <- c("#C5D86D","#00BFB2","#F1C40F","#004643")
mylabel <- c("Freshwater-adapted (N = 48)", "Cross-habitat (N = 28)", "Brackish-adapted (N = 44)")
...
ecotype_predict = function(x, quants = c(0.5, 0.05, 0.95), names = c("median", "lower", "upper")) {
	...
	# list of 4 ecotypes
	# each element is a matrix, 20000 rows (samples) by 15 age increments
	eco_pr = lapply(0:2, \(i) t(apply(samps[, c(1, 4, 7) + i], 1, Incpredict, Agei = 1:15)))
...
ggsave("Figures/Supplement/Individual_growth_ecotypes-revised.png", ecocurves3, width = 15, height = 15, units = "cm", dpi = 600, bg = "white")
```
...
pl_theme1 =	theme_classic() +
	theme(panel.grid.major = element_line(),
		  ...
		  plot.margin = unit(c(.8,0.15,-1.4,1), "cm"),
...
pl_theme3 =	theme_classic() +
	theme(panel.grid.major = element_line(),
		  ...
		  axis.title.y = element_blank(),
		  plot.title = element_text(size = 12, colour = "black"),
		  plot.margin = unit(c(-0.8,0.15,0,1), "cm"),
		  legend.position = "none")
...
ggsave("Figures/MS/PDF/Fig8.pdf", allplot2, width = 15, height = 15, units = "cm", dpi = 600, bg = "white")
```
...
ggsave("Figures/supplement/phenotypes-genetic-clusters-k4_hard-assigned.png", Hardclusts, width = 15, height = 9, units = "cm", dpi = 600, bg = "white")
```
