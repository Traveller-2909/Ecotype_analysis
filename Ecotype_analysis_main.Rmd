...
Agegrowth <- ggplot(Newdata2, aes(factor(lifeyear), log(prediction), fill=ecotype), alpha=0.6)+
  geom_boxplot(lwd=.1, outlier.shape = NA, width=.7, position = position_dodge(width = NULL), color="black")+
  labs(x="Alter (Jahre)", y="Zuwachs (log-Skala)",
       color="Verhaltenstyp", fill="Verhaltenstyp")+
  ...
  scale_fill_manual(breaks = c("BW-resident","Anadromous","Trans-habitat","FW_resident"),
                    values = mypalette1,
                    labels = c("Brackwasserresident", "Anadromer Typ","Übergangstyp", "Süßwasserresident"))+
  scale_fill_manual(breaks = c("BW-resident","Anadromous","Trans-habitat","FW_resident"),
                    values = mypalette1,
                    labels = c("Brackwasserresident", "Anadromer Typ","Übergangstyp", "Süßwasserresident"))+
  theme_classic()+
  theme(panel.grid.major = element_line(),
        legend.position = c(.71,.8),
        legend.background = element_blank(),
...
ggsave("Figures/MS/Ecotype-age-specific-growth-large_GER.png", Agegrowth, width = 20, height = 15, units = "cm", dpi = 600, bg = "white")
```
